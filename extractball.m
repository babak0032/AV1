...
function [cc,cr,radius,flag]=extractball(Imwork,Imback,fig1,fig2,fig3,fig15,index)
  ...
  stats = regionprops(labeled,['basic']); %Gets Area, centriod, and the bounding box
  [N,W] = size(stats);
  if N < 1
    return   
  end
  ...
  selected = (labeled==id(1) | labeled == id(2) | labeled == id(3) | labeled == id(4) | labeled == id(5) | labeled == id(6));
  if fig3 > 0
    figure(fig3)
    clf
    imshow(selected)
